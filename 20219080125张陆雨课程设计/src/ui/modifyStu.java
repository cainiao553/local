...
public class modifyStu extends JFrame implements ActionListener {
	...
	private JLabel tishi12;
	private JLabel id;
	private JTextField id_old;

	public modifyStu() {
		this.id = id;
		jframe = new JFrame("修改学生信息");
		jframe.setSize(600, 600);
		jframe.setLayout(null);

		id = new JLabel("修改前学号:");
		id.setSize(80, 20);
		id.setLocation(162, 20);
		jframe.add(id);
		
		id_old=new JTextField();
		id_old.setSize(200,20);
		id_old.setLocation(240, 22);
		jframe.add(id_old);
		
		tishi12=new JLabel();
		tishi12.setSize(80, 20);
		tishi12.setLocation(450, 20);
		jframe.add(tishi12);
	...
	public void actionPerformed(ActionEvent e) {
		...
		tishi11.setText("");
		boolean flag = true;
		if(id_old.getText().equals("")) {
			tishi12.setText("不能为空");
			flag=false;
		}else {
			tishi12.setText("");
		}
		...
		if (flag) {
			Student student = new Student(namet.getText(), idt.getText(), sext.getText().charAt(0),
					Integer.parseInt(oldt.getText()), Float.parseFloat(heightt.getText()),
					Double.parseDouble(weightt.getText()), telt.getText(), addt.getText(),
					Integer.parseInt(gradet.getText()), Boolean.parseBoolean(graduatet.getText()));

			if (service.修改(id_old.getText(),student)) {
				tishi11.setText("修改成功");
				namet.setText("");
				idt.setText("");
				sext.setText("");
				oldt.setText("");
